---
version: 1
slug: "src-app-kpi-cycle-page-tsx"
primary_target: "src/app/kpi/cycle/page.tsx"
related_targets: []
---
...
Locked for Operate craft (16 Sep 2026). Parent brief `docs/shape/cycle.md` sections 1–6 remain authoritative; builder-locked decisions below.

- **Nav and title:** Sidebar and page title **Cycle** (not KPI Admin). Domain object remains KpiCycle.
- **Structure:** One **CycleCard** per cycle — control strip (status, cadence when open, CheckIn window line, actions) stacked above a single assignment **Table** (~20–30 rows, horizontal scroll wrapper only). Not a dashboard; no hero metrics or stat tiles.
- **On-track:** Muted `text-sm text-muted` line directly under **PageHeader**, only when an open cycle exists; count from `kpiOnTrackCount`. Never a large number or primary focal block.
- **Actions:** Primary sage **Open** on closed cycles. On open cycles: secondary **Close and score**, secondary **Draft missing KpiSets (n)** only when `n > 0`. Cadence via **Field** + **Select** (monthly | quarterly), immediate `setCycleCadence`.
- **CheckIn windows:** Compact inline subline — `Q{n} open` for open quarters, `Q{n}` otherwise, dot-separated.
- **Roster columns:** Person legal name, Position title, KpiSet **StatusBadge** or “Missing”, **Health / score** — **HealthBadge** while cycle open; stored numeric **KpiScore** when cycle closed; em dash when no set or no score.
- **Guards:** Non–HR/tenant-admin → single warning **Callout** only (no header, roster, or controls). Empty `cycles` → calm accent **Callout** under header.
- **PageHeader:** Keeps kicker; description states open/close/score rules, no bonus math, and that per-item cadence override lives on My KPI.
- **Implementation:** `src/app/kpi/cycle/page.tsx` + `src/components/kpi/cycle/CycleCard.tsx`; store actions unchanged. Shell, nav, and other KPI routes parent-owned.
