---
version: 1
slug: "src-app-kpi-tree-page-tsx"
primary_target: "src/app/kpi/tree/page.tsx"
related_targets: []
---
...
**THESIS:** KPI tree is the open-cycle cascade forest on the existing quiet-sage KMPlus world — parent-pointer KpiItems across Assignments, not org chart or Portaverse layers; scan, spot Direct breaks, open KpiSet to fix elsewhere.

**OWN-WORLD:** Restrained sage from DESIGN.md: paper node cards, 1.5px line borders, scroll canvas on bg panel, solid sage Direct edges and dashed Indirect. PageHeader keeps cycle name in kicker. No navy chrome, fake tenant root, Impact/Output toggles, or mobile list fallback.

**STORY:** Land with cycle kicker → pick My team or Whole tenant (HR) → read edge legend → pan wide multi-root forest → notice Direct break badge or dashed branch → click node to open that Assignment’s KpiSet.

**FIRST VIEWPORT:** PageHeader, scope Field + inline legend, dominant bordered forest panel (or warning Callout when no open cycle, muted empty copy when scope has no nodes).

**FORM:** Header → scope + legend row → primary horizontal-scroll forest (SVG under absolute node cards). Node card: name, actual/target + unit, Direct/Indirect/Root, owner footer; warning Badge on invalid Direct child; whole card links to `/kpi/set/[id]` when permitted.

**FINISH:** Production Operate polish — hover/focus on links, `aria-hidden` SVG, labeled scope, ancestor pull in scoped views, Direct-break discoverability on the offending node, English CONTEXT.md nouns; cascade editing stays off-canvas.
